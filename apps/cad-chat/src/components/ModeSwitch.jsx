...
// demo:不開放的模式(lib/chatModes DEMO_HIDDEN_MODES,目前是無塵電纜)整段不渲染——
// 這是「藏」不是「禁用」:展示身分不該知道有客戶案件工作流。安全邊界在 server。
    ...
    <div
      ...
      title={
        running
          ? "回合進行中,結束後才能切換模式"
          : show("cable")
            ? "草模=快速機構運動示意;設計=真幾何 CAD;無塵電纜=電纜組長寬高參數化重建;零件庫=收 STP 進庫/查庫(切換會開新對話)"
            : "草模=快速機構運動示意;設計=真幾何 CAD;零件庫=收 STP 進庫/查庫(切換會開新對話)"
      }
    >
