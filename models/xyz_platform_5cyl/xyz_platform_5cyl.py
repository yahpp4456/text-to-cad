"""XYZ platform (C-frame gantry) with a 5-cylinder pneumatic head pointing down.
...
Z carriage carries a head plate with FIVE independent pneumatic cylinders
(SC-series, also selected); body and rod point straight down; each cylinder
strokes its nozzle independently.

Layout (C-frame so the head can descend into open space):
    base -> X axis (flat on the base, travel +X)
         ...
         -> head plate cantilevered into open +Y air -> 5 cylinders (rods down)
         -> 5 nozzles
...
8 DOF: x, y, z (axis travels, mm) and e0..e4 (cylinder extensions, mm). pose() is
...
# Five cylinders: each sized off a light per-nozzle pick/handling load. The
# catalog's smallest cylinder is SC32 (body dia 42); a real micro-nozzle head
# would use a far smaller bore not present in this table, so the head is sized
# AROUND the SC32 envelope (reported as the catalog floor, not a fit claim).
N_NOZZLES = 5
CYL_STROKE = 25.0
...
# PARAMS: cad-chat slider surface (stroke values mirror selected axis duty).
PARAMS = {
    "x_stroke": float(X_STROKE),
    "y_stroke": float(Y_STROKE),
    "z_stroke": float(Z_STROKE),
    "cyl_stroke": float(CYL_STROKE),
    "n_cylinders": float(N_NOZZLES),
}
...
# ===========================================================================
# Head (5 independent pneumatic cylinders, rods down)
# ===========================================================================
def _nozzle():
...
def _head_parts(x, y, z, e):
    ...
    plate_top = under + _HEAD_PLATE_T
    out = []

    # Wide bar under the 5-cylinder row (rods hang straight down from underside).
    bar_x0 = cx0 - NOZZLE_PITCH * 0.5
    bar_x1 = cx0 + (N_NOZZLES - 1) * NOZZLE_PITCH + NOZZLE_PITCH * 0.5
    plate_y0 = ny - 15.0
    plate_y1 = ny + 15.0
    out.append(("head_plate", _box(bar_x0, bar_x1, plate_y0, plate_y1, under, plate_top)))

    # Arm on Z-carriage centreline (clears z_base / rails). Sits ON TOP of the
    # head plate so it never occupies the centre cylinder's body volume (cyl2 is
    # embedded only a few mm into the plate underside and hangs down).
    arm_y0 = _ZC_Y + y + 18.0
    arm_y1 = ny + 8.0
    out.append((
        "head_arm",
        _box(_ZC_X + x - 8.0, _ZC_X + x + 8.0, arm_y0, arm_y1, plate_top, _ZC_BOT - z + 6.0),
    ))
...
def pose(x=0.0, y=0.0, z=0.0, e=_E0):
    """All parts (name, shape) at gantry state (x, y, z mm; e = N_NOZZLES cylinder mm)."""
    parts = []
...
def check_geometry(shape):
    ...
    per-axis whole-travel motion sweep for X, Y, Z and the 5-cylinder down-stroke.
...
# Ride-along labels for cad-chat MOTION (mirrors pose() kinematics).
_HEAD = ["head_plate", "head_arm"]
_X_MOVING = (
    [f"x_{n}" for n in _STAGE_MOVING]
    + ["riser"]
    + [f"y_{n}" for n in _Y]
    + ["z_bracket"]
    + [f"z_{n}" for n in _Z]
    + _HEAD
    + [f"cyl{i}_body" for i in range(N_NOZZLES)]
    + [f"cyl{i}_rod" for i in range(N_NOZZLES)]
    + [f"nozzle{i}" for i in range(N_NOZZLES)]
)
_Y_MOVING = (
    [f"y_{n}" for n in _STAGE_MOVING]
    + ["z_bracket"]
    + [f"z_{n}" for n in _Z]
    + _HEAD
    + [f"cyl{i}_body" for i in range(N_NOZZLES)]
    ...
    "dofs": [
        {
            ...
            "type": "linear",
            "axis": [1, 0, 0],
            "travel": PARAMS["x_stroke"],
            "moving": _X_MOVING,
            "pairs": [[a, b] for a, b in _X_PAIRS],
            "samples": 8,
        },
        {
            "id": "y",
            ...
            "type": "linear",
            "axis": [0, 1, 0],
            "travel": PARAMS["y_stroke"],
            "moving": _Y_MOVING,
            "pairs": [[a, b] for a, b in _Y_PAIRS],
            "samples": 8,
        },
        {
            "id": "z",
            ...
            "type": "linear",
            "axis": [0, 0, -1],
            "travel": PARAMS["z_stroke"],
            "moving": _Z_MOVING,
            "pairs": [[a, b] for a, b in _Z_PAIRS],
            "samples": 8,
        },
        *[
            {
                ...
                "type": "linear",
                "axis": [0, 0, -1],
                "travel": PARAMS["cyl_stroke"],
                "moving": [f"cyl{i}_rod", f"nozzle{i}"],
                "pairs": [[f"cyl{i}_rod", f"cyl{i}_body"], [f"nozzle{i}", "head_plate"]],
                "samples": 6,
            }
            for i in range(N_NOZZLES)
        ],
...
def gen_step():
    asm = AssemblyHelper("xyz_platform_5cyl")
    parts = {}
    ...
    for i in range(N_NOZZLES):
        xi = _HEAD_X0 + i * NOZZLE_PITCH
        asm.linear_frame(
            parts[f"nozzle{i}"],
            f"cyl{i}_stroke",
            Axis((xi, _NOZZLE_Y0, _HEAD_UNDER0), (0.0, 0.0, -1.0)),
        )
    return asm.build()
